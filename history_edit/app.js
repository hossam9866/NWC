...
function setEditHistoryData(log) {
  ...
  actorName.innerText = `${log["actorName"]} ${log["actorId"]} Edited On: `;
  ...
  for (let i = 0; i < log["metadata"]["newValues"].length; i++) {
    ...
    if (oldValue != newValue) {
      ...
      const regExp =
        /2[0-9][0-9][0-9]-[0-3][0-9]-[0-3][0-9]T[0-2][0-9]:[0-6][0-9]:[0-6][0-9][0-9]?[+][0-9][0-9]:[0-9][0-9]/;
      if (regExp.test(oldValue) || regExp.test(newValue)) {
        changeElement.innerText = `Changed ${fieldName} from (${
          !regExp.test(oldValue) ? "Not Declared" : showDate(new Date(oldValue))
        } to ${
          !regExp.test(newValue) ? "Not Declared" : showDate(new Date(newValue))
        })`;
      } else {
